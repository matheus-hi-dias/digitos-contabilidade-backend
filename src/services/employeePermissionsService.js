...
const selectByEmployeeId = async (employee_id) => {
  const employeePermissions = await employeePermissionRepository.findByEmployeeId(employee_id);
  ...
  const employeePermissionsFormatted = await Promise.all(employeePermissions.map(async (employeePermission) => {
    const permissionById = await permissionsService.selectById(employeePermission.permission_id);
...
const create = async (employeePermission) => {
  ...
  const employee = await employeesService.selectById(employeePermission.employee_id);
  if (!employee) {
    throw makeError({message: "Employee not found", status: 404});
  }

  const permission = await permissionsService.selectById(employeePermission.permission_id);
  if (!permission) {
...
const remove = async (employee_id, permission_id) => {
  const employeePermission = await employeePermissionRepository.deleteEmployeePermission(employee_id, permission_id);
